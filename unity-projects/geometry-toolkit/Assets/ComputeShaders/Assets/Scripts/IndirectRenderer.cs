...
public class IndirectRenderer : MonoBehaviour
{
    ...
    private void OnDestroy()
    {
        ...
        if (debugDrawLOD)
        {
            for (var i = 0; i < indirectMeshes.Length; i++)
            {
    ...
    private void OnDrawGizmos()
    {
        ...
        if (debugDrawBoundsInSceneView)
        {
            ...
            for (var i = 0; i < instancesInputData.Count; i++)
            {
    ...
    private void DrawInstances()
    {
        for (var i = 0; i < indirectMeshes.Length; i++)
        {
            var argsIndex = i * ARGS_BYTE_SIZE_PER_INSTANCE_TYPE;
            var irm = indirectMeshes[i];
    ...
    private void CreateSortingCommandBuffer()
    {
        ...
        var NUM_ELEMENTS = (uint)m_numberOfInstances;
        var MATRIX_WIDTH = BITONIC_BLOCK_SIZE;
        var MATRIX_HEIGHT = (uint)NUM_ELEMENTS / BITONIC_BLOCK_SIZE;
        ...
        for (var level = (BITONIC_BLOCK_SIZE << 1); level <= NUM_ELEMENTS; level <<= 1)
        {
            // Transpose the data from buffer 1 into buffer 2
            var l = (level / BITONIC_BLOCK_SIZE);
            var lm = (level & ~NUM_ELEMENTS) / BITONIC_BLOCK_SIZE;
    ...
    private bool InitializeRenderer(ref IndirectInstanceData[] _instances)
    {
        ...
        for (var i = 0; i < m_numberOfInstanceTypes; i++)
        {
            var irm = new IndirectRenderingMesh();
            var iid = _instances[i];
            ...
            // Arguments
            var argsIndex = i * NUMBER_OF_ARGS_PER_INSTANCE_TYPE;
            ...
            var originalBounds = CalculateBounds(iid.prefab);
            
            // Add the instance data (positions, rotations, scaling, bounds...)
            for (var j = 0; j < iid.positions.Length; j++)
            {
        ...
        var computeShaderInputSize = Marshal.SizeOf(typeof(IndirectInstanceCSInput));
        var computeShaderDrawMatrixSize = Marshal.SizeOf(typeof(Indirect2x2Matrix));
        var computeSortingDataSize = Marshal.SizeOf(typeof(SortingData));
        ...
        var _Whatever = Shader.PropertyToID("_Whatever");
        var _DebugLODEnabled = Shader.PropertyToID("_DebugLODEnabled");
        for (var i = 0; i < indirectMeshes.Length; i++)
        {
            var irm = indirectMeshes[i];
            var argsIndex = i * NUMBER_OF_ARGS_PER_INSTANCE_TYPE;
        ...
        var positionsBuffer = new ComputeBuffer(m_numberOfInstances, Marshal.SizeOf(typeof(Vector3)), ComputeBufferType.Default);
        var scaleBuffer = new ComputeBuffer(m_numberOfInstances, Marshal.SizeOf(typeof(Vector3)), ComputeBufferType.Default);
        var rotationBuffer = new ComputeBuffer(m_numberOfInstances, Marshal.SizeOf(typeof(Vector3)), ComputeBufferType.Default);
        ...
        var groupX = Mathf.Max(1, m_numberOfInstances / (2 * SCAN_THREAD_GROUP_SIZE));
    ...
    private void UpdateDebugUI()
    {
        ...
        {
            var argsBuffer = m_debugGPUArgsRequest.GetData<uint>();
            var shadowArgsBuffer = m_debugGPUShadowArgsRequest.GetData<uint>();
            ...
            var instanceIndex = 0;
            var normMultiplier = (uint) (drawInstances ? 1 : 0);
            var shadowMultiplier = (uint) (drawInstanceShadows && QualitySettings.shadows != ShadowQuality.Disable ? 1 : 0);
            ...
            for (var i = 0; i < argsBuffer.Length; i = i + NUMBER_OF_ARGS_PER_INSTANCE_TYPE)
            {
                var irm = indirectMeshes[instanceIndex];
                
                var lod00Count = argsBuffer[i +  1] * normMultiplier;
                var lod01Count = argsBuffer[i +  6] * normMultiplier;
                var lod02Count = argsBuffer[i + 11] * normMultiplier;
                
                var lod00ShadowCount = shadowArgsBuffer[i +  1] * shadowMultiplier;
                var lod01ShadowCount = shadowArgsBuffer[i +  6] * shadowMultiplier;
                var lod02ShadowCount = shadowArgsBuffer[i + 11] * shadowMultiplier;
                
                var lod00Indices = argsBuffer[i +  0] * normMultiplier;
                var lod01Indices = argsBuffer[i +  5] * normMultiplier;
                var lod02Indices = argsBuffer[i + 10] * normMultiplier;
                
                var shadowLod00Indices = shadowArgsBuffer[i +  0] * shadowMultiplier;
                var shadowLod01Indices = shadowArgsBuffer[i +  5] * shadowMultiplier;
                var shadowLod02Indices = shadowArgsBuffer[i + 10] * shadowMultiplier;
                
                var lod00Vertices = irm.numOfVerticesLod00 * normMultiplier;
                var lod01Vertices = irm.numOfVerticesLod01 * normMultiplier;
                var lod02Vertices = irm.numOfVerticesLod02 * normMultiplier;
                
                var shadowLod00Vertices = irm.numOfVerticesLod00 * shadowMultiplier;
                var shadowLod01Vertices = irm.numOfVerticesLod01 * shadowMultiplier;
                var shadowLod02Vertices = irm.numOfVerticesLod02 * shadowMultiplier;
                
                // Output...
                var lod00VertColor = (lod00Vertices > 10000 ? DEBUG_UI_RED_COLOR : DEBUG_UI_WHITE_COLOR);
                var lod01VertColor = (lod01Vertices > 5000 ? DEBUG_UI_RED_COLOR : DEBUG_UI_WHITE_COLOR);
                var lod02VertColor = (lod02Vertices > 1000 ? DEBUG_UI_RED_COLOR : DEBUG_UI_WHITE_COLOR);
                
                var lod00IndicesColor = (lod00Indices > (lod00Vertices * 3.33f) ? DEBUG_UI_RED_COLOR : DEBUG_UI_WHITE_COLOR);
                var lod01IndicesColor = (lod01Indices > (lod01Vertices * 3.33f) ? DEBUG_UI_RED_COLOR : DEBUG_UI_WHITE_COLOR);
                var lod02IndicesColor = (lod02Indices > (lod02Vertices * 3.33f) ? DEBUG_UI_RED_COLOR : DEBUG_UI_WHITE_COLOR);
                ...
                // Total
                var sumCount = lod00Count + lod01Count + lod02Count;
                var sumShadowCount = lod00ShadowCount + lod01ShadowCount + lod02ShadowCount;
                
                var sumLod00Indices = lod00Count * lod00Indices;
                var sumLod01Indices = lod01Count * lod01Indices;
                var sumLod02Indices = lod02Count * lod02Indices;
                var sumIndices = sumLod00Indices + sumLod01Indices + sumLod02Indices;
                
                var sumShadowLod00Indices = lod00ShadowCount * shadowLod00Indices;
                var sumShadowLod01Indices = lod01ShadowCount * shadowLod01Indices;
                var sumShadowLod02Indices = lod02ShadowCount * shadowLod02Indices;
                var sumShadowIndices = sumShadowLod00Indices + sumShadowLod01Indices + sumShadowLod02Indices;
                
                var sumLod00Vertices = lod00Count * lod00Vertices;
                var sumLod01Vertices = lod01Count * lod01Vertices;
                var sumLod02Vertices = lod02Count * lod02Vertices;
                var sumVertices = sumLod00Vertices + sumLod01Vertices + sumLod02Vertices;
                
                var sumShadowLod00Vertices = lod00ShadowCount * shadowLod00Vertices;
                var sumShadowLod01Vertices = lod01ShadowCount * shadowLod01Vertices;
                var sumShadowLod02Vertices = lod02ShadowCount * shadowLod02Vertices;
                var sumShadowVertices = sumShadowLod00Vertices + sumShadowLod01Vertices + sumShadowLod02Vertices;
    ...
    private void UpdateDebugUI1()
    {
        ...
        else if (m_debugGPUArgsRequest.done && m_debugGPUShadowArgsRequest.done)
        {
            ...
            for (var i = 0; i < argsBuffer.Length; i = i + NUMBER_OF_ARGS_PER_INSTANCE_TYPE)
            {
                ...
                var lod00Vertices = (uint)irm.numOfVerticesLod00 * normMultiplier;
                var lod01Vertices = (uint)irm.numOfVerticesLod01 * normMultiplier;
                var lod02Vertices = (uint)irm.numOfVerticesLod02 * normMultiplier;
                
                var shadowLod00Vertices = (uint)irm.numOfVerticesLod00 * shadowMultiplier;
                var shadowLod01Vertices = (uint)irm.numOfVerticesLod01 * shadowMultiplier;
                var shadowLod02Vertices = (uint)irm.numOfVerticesLod02 * shadowMultiplier;
    ...
    {
        var sortingData = new SortingData[m_numberOfInstances];
    ...
    {
        var matrix1 = new Indirect2x2Matrix[m_numberOfInstances];
        var matrix2 = new Indirect2x2Matrix[m_numberOfInstances];
        var matrix3 = new Indirect2x2Matrix[m_numberOfInstances];
    ...
    {
        var instancesArgs = new uint[m_numberOfInstanceTypes * NUMBER_OF_ARGS_PER_INSTANCE_TYPE];
        var shadowArgs = new uint[m_numberOfInstanceTypes * NUMBER_OF_ARGS_PER_INSTANCE_TYPE];
        ...
        for (var i = 0; i < instancesArgs.Length; i++)
        {
            ...
            if ((i + 1) % 5 == 0)
            {
                    ...
                    && (i + 1) % NUMBER_OF_ARGS_PER_INSTANCE_TYPE == 0)
                {
                    ...
                    counter++;
                    var irm = indirectMeshes[counter];
                    var m = irm.mesh;
    ...
    {
        var instancesIsVisible = new uint[m_numberOfInstances];
        var shadowsIsVisible = new uint[m_numberOfInstances];
    ...
    {
        var instancesScannedData = new uint[m_numberOfInstances];
        var shadowsScannedData = new uint[m_numberOfInstances];
    ...
    {
        var instancesMatrix1 = new Indirect2x2Matrix[m_numberOfInstances];
        var instancesMatrix2 = new Indirect2x2Matrix[m_numberOfInstances];
        var instancesMatrix3 = new Indirect2x2Matrix[m_numberOfInstances];
        ...
        var shadowsMatrix1 = new Indirect2x2Matrix[m_numberOfInstances];
        var shadowsMatrix2 = new Indirect2x2Matrix[m_numberOfInstances];
        var shadowsMatrix3 = new Indirect2x2Matrix[m_numberOfInstances];
        ...
        for (var i = 0; i < instancesMatrix1.Length; i++)
        {
            instancesSB.AppendLine(
                i + "\n" 
